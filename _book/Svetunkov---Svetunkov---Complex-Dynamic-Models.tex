...
\end{equation}
which is the square of the magnitude \eqref{eq:complexNumberMagnitude} of both complex variables \(\underline{z}\) and \(\tilde{\underline{z}}\). Conjugation is distributive over addition, subtraction, multiplication and division, meaning that:
\begin{equation}
    \begin{aligned}
...
\begin{verbatim}
##          x_r      x_i
## x_r 27.99580 -4.95314
## x_i -4.95314 93.16629
\end{verbatim}
...
\begin{verbatim}
##                     x                   y
## x -96.71601- 4.58636i -49.15862+84.27777i
## y -49.15862+84.27777i  65.05185+85.73248i
\end{verbatim}
...
\hypertarget{elliptic-uniform-distribution}{%
\subsection{Elliptic uniform distribution}\label{elliptic-uniform-distribution}}
...
\hypertarget{rectangular-uniform-distribution}{%
\subsection{Rectangular uniform distribution}\label{rectangular-uniform-distribution}}
...
\hypertarget{hotellings-distribution}{%
\subsection{Hotelling's distribution}\label{hotellings-distribution}}
...
\begin{equation}
    ...
    \label{eq:SimpleCLRVector01}
\end{equation}
where \(\mathbf{y}=\begin{pmatrix} {y}_1 \\ {y}_2\\ \vdots \\ {y}_n \end{pmatrix}\), \(\mathbf{X} = \begin{pmatrix} 1 & {x}_1 \\ 1 & {x}_2\\ \vdots \\ 1 & {x}_n \end{pmatrix}\), \(\boldsymbol{\beta} = \begin{pmatrix} {\beta}_0 \\ {\beta}_1 \end{pmatrix}\) and \(\boldsymbol{\epsilon} = \begin{pmatrix} {\epsilon}_1 \\ {\epsilon}_2\\ \vdots \\ {\epsilon}_n \end{pmatrix}\), where each element of the objects above is a complex number.
...
As for the variance of \(b_1\), it can be shown to be equal to:
\begin{equation}
...
\end{equation}
we do not expand it further, because the formula \eqref{eq:SimpleCLROLSb1Variance} suffices for the comparison of OLS with other estimators.
...
\end{equation}
Similarly to how it was done with OLS estimate of the slope, we can consider the estimate of \(b_1\), expanding the covariance in the numerator of \eqref{eq:SimpleCLRCLSLossParametersMoments}:
\begin{equation}
        b_{1} = \frac{\mathrm{cov}(x,y)}{V(x)} = \frac{\mathrm{cov}(x,\beta_0 + \beta_1 x + \epsilon)}{V(x)},
    \label{eq:SimpleCLRCLSb1Expansion01}
\end{equation}
which after some simplifications becomes:
\begin{equation}
        b_{1} = \beta_1 + \frac{\mathrm{cov}(x, \epsilon)}{V(x)} ,
    \label{eq:SimpleCLRCLSb1Expansion02}
\end{equation}
which can be expanded to:
\begin{equation}
    b_{1} = \beta_1 + \frac{\mathrm{cov}(x_r, \epsilon_r) - \mathrm{cov}(x_i, \epsilon_i) + i (\mathrm{cov}(x_r, \epsilon_i) + \mathrm{cov}(x_i, \epsilon_r))}{V(x_r) - V(x_i) + 2i \mathrm{cov}(x_r, x_i)} . 
...
\resizebox{0.9\textwidth}{!}{$
        b_{1} = \beta_1 + \frac{\left(\mathrm{cov}(x_r, \epsilon_r) - \mathrm{cov}(x_i, \epsilon_i) + i (\mathrm{cov}(x_r, \epsilon_i) + \mathrm{cov}(x_i, \epsilon_r))\right)\left(V(x_r) - V(x_i) - 2i \mathrm{cov}(x_r, x_i)\right)}{\left(V(x_r) - V(x_i)\right)^2 + 4 \mathrm{cov}(x_r, x_i)^2} $}.
...
\item
  When the complex variables \(x\) and \(y\) have functional relation between them, so that \(a_1 = \frac{1}{b_1}\), the coefficient will be equal to one.
\end{enumerate}

Note however that due to the division by a complex number, the value of 1 can also be obtained due to the values of direct variance of variables \(x\) and/or \(y\).
...
\begin{verbatim}
##            x_r         x_i         y_r        y_i
## x_r 1.00000000  0.02179449  0.72361210 0.74560977
## x_i 0.02179449  1.00000000 -0.67194989 0.67976014
## y_r 0.72361210 -0.67194989  1.00000000 0.08337813
## y_i 0.74560977  0.67976014  0.08337813 1.00000000
\end{verbatim}
...
So, the main difference of the form \eqref{eq:CLRSystemVectorComplexFinal} from the \eqref{eq:CLRSystemVectorFinal} is that it contains complex numbers inside each of the matrices and vectors.

\hypertarget{estimation}{%
\section{Estimation}\label{estimation}}

In order to estimate the parameters of the model \eqref{eq:CLRSystemVectorFinal}, we can use the same methods as in the Chapter \ref{simpleCLR}: OLS, CLS and Likelihood. We will write the estimated model as:
\begin{equation}
...
\end{equation}
Taking derivative of \eqref{eq:CLROLSCriterionExpanded} with respect to \(\boldsymbol{b}\) and equating it to zero, results in the following system of normal equations:
\begin{equation}
...
\end{equation}
For the simple CLR, the formula \eqref{eq:MCLRCLSEstimate} becomes equivalent to \eqref{eq:SimpleCLRCLSLossParameters}. The main difference between \eqref{eq:MCLRCLSEstimate} and \eqref{eq:MCLROLSEstimateComplex}, as we can see, is that the CLS does not do multiplication by the conjugate complex numbers.

Finally, using the direct multiplication \eqref{eq:complexNumberMatrixMultiDirect}, it can be shown that the estimates of parameters according to CLS, can be calculated as:
\begin{equation}
