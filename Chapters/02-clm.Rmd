...
```{r crvCorConjugate, fig.width=4, fig.height=4, fig.cap="Visualisation of relations between two complex variables"}
# Create a c.r.v. x
x <- complex(real=rnorm(1000,0,10), imaginary=rnorm(1000,0,10))
# Create a c.r.v. y
y <- (10 + 15i) + (1 + 1i) * x +
    complex(real=rnorm(1000,0,1), imaginary=rnorm(1000,0,1))
# Produce the plot
cplot(x, y)
# Conjugate correlation
ccor(x, y, kind="conjugate")
```

As can be seen from the Figure \@ref(fig:crvCorConjugate) and the value of the conjugate correlation, there is a relation between the two complex variables $x$ and $y$, but it although the value of the latter is approximately `r round(ccor(x, y, kind="conjugate"),3)`, we cannot conclude unambiguously that the relation is strong.
