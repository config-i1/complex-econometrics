...
\end{equation}
we do not expand it further, because the formula \@ref(eq:SimpleCLROLSb1Variance) suffices for the comparison of OLS with other estimators.
...
\end{equation}
Note that both numerator and denominator of \@ref(eq:SimpleCLRCLSb1Expansion03) are complex numbers. In order to have a proper split into real and imaginary parts we need to multiply the fraction by the complex number conjugate to the denominator:
\begin{equation}
...
\end{equation}
Comparing \@ref(eq:SimpleCLRCLSb1Variance) with \@ref(eq:SimpleCLROLSb1Variance), we can conclude that the estimates of parameters via CLS are in general less than or equally efficient as the estimates via OLS. There are some cases, when the variance \@ref(eq:SimpleCLRCLSb1Variance) is much higher than \@ref(eq:SimpleCLROLSb1Variance). For example, when the real and imaginary parts of $x$ are not correlated (i.e. $\hat{\sigma}_{x_r, x_i}=0$) and when the variances of the real and imaginary parts of $x$ are equal, the variance of $b_{1,r}$ explodes. In the opposite situation, when $\hat{\sigma}_{x_r, x_i} \neq 0$ and $\hat{\sigma}_{x_r}^2 \neq \hat{\sigma}_{x_i}^2$, the variance \@ref(eq:SimpleCLRCLSb1Variance) will typically be greater or equal to the variance of \@ref(eq:SimpleCLROLSb1Variance). So, we can conclude that in general CLS produces less efficient estimates of parameters than OLS for the simple CLR.
