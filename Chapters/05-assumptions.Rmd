...
y[1] <- 300 + 250i
for(i in 2:obs){
    y[i] <- 100 + 200i + (0.5 - 0.25i) * y[i-1] + complex(real=rnorm(1,0,10), imaginary=rnorm(1,0,5))
}
```
...
pcacf(y, method="conjugate", plot=TRUE, main="")
```
...
pcacf(y, method="direct", plot=TRUE, main="")
```
...
pcacf(y, method="pearson", plot=TRUE)
```
...
Last but not least, polar coordinates...


*Focus here on diagnostics, don't solve the problem*. The solution is in Chapter \@ref(ComplexAR)
