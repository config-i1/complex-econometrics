...
{\centering \includegraphics[width=0.5\linewidth]{./images/complex} 

}

\caption{Hex-sticker of the complex package in R.}\label{fig:complexPackage}
\end{figure}
...
\end{equation}
where \(e\) is the Euler's constant. Equation \eqref{eq:complexNumberExponential} is nowadays also called ``Euler's formula''. Comparing \eqref{eq:complexNumberTrigonometric} with \eqref{eq:complexNumberExponential}, we can conclude that:
\begin{equation}
...
\begin{verbatim}
##                      x                   y
## x -60.19156- 5.694255i -51.28506+50.82070i
## y -51.28506+50.820699i  12.71148+48.05904i
\end{verbatim}
...
\begin{equation}
    K + iL .
    \label{eq:CVPFexplanatory}
\end{equation}

If we now estimate a function of the complex variable \eqref{eq:CVPFresponse} from \eqref{eq:CVPFexplanatory}, we will model complex interactions between labour, capital, expenses and profit, including the interactions between the capital-labour ration and the profitability. This setting provides an example of motivation of modelling economic process using complex variables.
...
We start with the visual analysis of relations, then move to the discussion of different types of correlation coefficients, sequentially discussing the conjugate, direct, Pearson's coefficients and finish the chapter with the discussion of correlation matrix of complex variables.

\hypertarget{correlationVisual}{%
\section{Visualisation of relations}\label{correlationVisual}}

To better understand what correlations between c.r.v. imply, we need to understand how to visualise them first. The conventional way of doing that is using scatterplots. While in case of two real variables it is straightforward (a variable per axe), in complex-valued domain, this task has several potential solutions. The first one is to consider a set of scatterplots as shown in Figure \ref{fig:crvScatterplots} for two complex random variables \(\underline{x}\) and \(\underline{y}\), created using \texttt{cplot()} function from \texttt{complex} package in R.
...
We now move to the discussion of the multiple cLR, the model that captures relations between one complex random variable, \(y_r + i y_i\) and a set of explanatory complex random variables. In this chapter, after discussing the model formulation we will move to the question of estimation, then to inference based on cLR, discuss how to correctly capture uncertainty in the model and finally how to include dummy variables in it.
...
\begin{figure}
\centering
\includegraphics{Svetunkov---Svetunkov---Complex-Valued-Econometrics_files/figure-latex/nonLinearExample-1.pdf}
\caption{\label{fig:nonLinearExample}Non-linear transformation of a complex variable that changes linearly.}
\end{figure}
...
In this chapter we consider examples of application of cLR and cARIMA models to two separate problems. The first one is related to sales forecasting, while the second one focuses on the analysis of efficiency of policies.
...
Finally, we focused this monograph on complex-valued models that have two real valued variables in the complex response one. But this is not the only way of modelling using complex variables. \citet{Svetunkov2012} discussed some other ways of modelling which lie outside of this monograph.
