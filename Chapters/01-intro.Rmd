...
\end{equation}
where $e$ is the Euler's constant. The formula \@ref(eq:complexNumberExponential) is nowadays also called "Euler's formula". In this form, the positive real number has $\phi=0$, while the negative one has $\phi=\pi$, and all imaginary numbers have an angle dividable by $\frac{\pi}{2}$. So, for example:
\begin{equation*}
    \begin{aligned}
    41 = & 41 e^{i 0} \\
...
In a way, any mathematical operation with a complex number implies a change in magnitude and angle of the number, while the multiplication and division by a number with non-zero imaginary part will always lead to the rotation of the number. In fact, the operations of multiplication and division are easier to do in either exponential or trigonometric forms of complex variables:
\begin{equation*}
...
```{r complexCartesianvsPolar, fig.height=4, fig.cap="Visualisation of complex variables on Cartesian and on polar coordinates planes.", echo=FALSE}
x <- rnorm(100,100,10)
...
When it comes to operations with complex numbers, mathematically we will say that two complex numbers $z_1 = x_1 + i y_1$ and $z_2 = x_2 + i y_2$ are equal to each other if and only if their real and imaginary parts are equal:
\begin{equation*}
    ...
    \right. ,
\end{equation*}
which is equivalent to saying that their magnitudes and polar angles are the same:
\begin{equation*}
    z_1 = z_2 \iff \left \lbrace
    \begin{aligned}
        & |z_1| = |z_2| \\
