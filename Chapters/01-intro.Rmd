...
\end{equation}
These representations become especially useful when we work with complex variables to construct complex models. Finally, in case of matrix form, it is possible to multiply complex variable by itself without the conjugation, which results in:
\begin{equation}
    \begin{pmatrix} x & -y \\ y & x \end{pmatrix} \times \begin{pmatrix} x & -y \\ y & x \end{pmatrix} = 
...
\begin{equation}
    \mathbf{x} = \begin{pmatrix} x_{r,1} & - x_{i,1} \\ x_{i,1} & x_{r,1} \\
                                 x_{r,2} & - x_{i,2} \\ x_{i,2} & x_{r,2} \\
                                 x_{r,3} & - x_{i,3} \\ x_{i,3} & x_{r,3} \end{pmatrix} .
    (\#eq:complexMatrix)
\end{equation}
Each of the forms \@ref(eq:complexVector), \@ref(eq:complexVectorMatrix) and \@ref(eq:complexMatrix) might be useful in different circumstances, their usage should be dictated by the modelling purpose.
...
and
\begin{equation}
    \mathbf{x}^{\top} = \begin{pmatrix} x_{r,1} & - x_{i,1} & x_{r,2} & - x_{i,2} & x_{r,3} & - x_{i,3} \\
                                        x_{i,1} & x_{r,1} & x_{i,2} & x_{r,2} & x_{i,3} & x_{r,3}
                        \end{pmatrix} .
    (\#eq:complexMatrixTransposed)
\end{equation}
We will denote the operation of transposition above with the $\top$ symbol and will call it "complex transposition" as opposed to the original $\prime$ and conventional transposition.
