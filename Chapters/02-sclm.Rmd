...
<!--                           & \mathrm{E}\left(\left(\beta_1 + \frac{\hat{\sigma}_{x,\epsilon}}{\hat{\sigma}_x^2} - \beta_1 \right) \left(\tilde{\beta}_1 + \frac{\hat{\sigma}_{\epsilon,x}}{\hat{\sigma}_x^2} - \tilde{\beta}_1 \right)\right) = \\ -->
...
\end{equation}
Similarly to how it was done with OLS estimate of the slope, we can consider the estimate of $b_1$, expanding the covariance in the numerator of \@ref(eq:SimpleCLRCLSLossParametersMoments):
\begin{equation}
        b_{1} = \frac{\mathrm{cov}(x,y)}{V(x)} = \frac{\mathrm{cov}(x,\beta_0 + \beta_1 x + \epsilon)}{V(x)},
    (\#eq:SimpleCLRCLSSlopeExpansion01)
\end{equation}
which after some simplifications becomes:
\begin{equation}
        b_{1} = \beta_1 + \frac{\mathrm{cov}(x, \epsilon)}{V(x)} ,
    (\#eq:SimpleCLRCLSSlopeExpansion02)
\end{equation}
...
\begin{equation}
    ...
    (\#eq:SimpleCLRCLSSlopeExpansion03)
\end{equation}
Note that both numerator and denominator of \@ref(eq:SimpleCLRCLSSlopeExpansion03) are complex numbers. In order to have a proper split into real and imaginary parts we need to multiply the fraction by the complex number conjugate to the denominator:
\begin{equation}
    ...
    (\#eq:SimpleCLRCLSSlopeExpansion04)
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
    (\#eq:SimpleCLRCLSSlopeExpansion05)
\end{equation}
or
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
    (\#eq:SimpleCLRCLSSlopeExpansion06)
\end{equation}
If we now consider the properties of $b_{1,r}$, we can see that it is an asymptotically unbiased estimate of $\beta_{1,r}$ when the basic regression assumptions hold (i.e. ${\sigma}_{x_r, \epsilon_r} = {\sigma}_{x_i, \epsilon_i} = {\sigma}_{x_r, \epsilon_i} = {\sigma}_{x_i, \epsilon_r}=0$).

$\begin{aligned} b_{1,r} = & \beta_{1,r} + \frac{\left(\hat{\sigma}_{x_r, \epsilon_r} - \hat{\sigma}_{x_i, \epsilon_i}\right) \left(\hat{\sigma}_{x_r}^2 - \hat{\sigma}_{x_i}^2 \right) - 2 \hat{\sigma}_{x_r, x_i} \left(\hat{\sigma}_{x_r, \epsilon_i} + \hat{\sigma}_{x_i, \epsilon_r}\right)}{\left(\hat{\sigma}_{x_r}^2 - \hat{\sigma}_{x_i}^2\right)^2 + 4 \hat{\sigma}_{x_r, x_i}^2} \\ b_{1,i} = & \beta_{1,i} + \frac{\left( \hat{\sigma}_{x_r, \epsilon_i} + \hat{\sigma}_{x_i, \epsilon_r}\right)\left(\hat{\sigma}_{x_r}^2 - \hat{\sigma}_{x_i}^2 \right) + 2 \hat{\sigma}_{x_r, x_i} \left(\hat{\sigma}_{x_i, \epsilon_i} - \hat{\sigma}_{x_r, \epsilon_r}\right)}{\left(\hat{\sigma}_{x_r}^2 - \hat{\sigma}_{x_i}^2\right)^2 + 4 \hat{\sigma}_{x_r, x_i}^2} \end{aligned}$
...
1. **Derive expectation of estimator**

2. **Derive variance of estimator**
...
## Comparing different estimators for SCLR
...
obs <- 10000
x0 <- rnorm(obs,10,10)
x <- complex(real=x0,imaginary=rnorm(obs,0,10))
...
parametersPlots <- function(data){
    ...
    plot(Re(data[,2,2]), xlab="Sample size", ylab=TeX("Re($b_1$)"),
    ...
    plot(Re(data[,1,2]), xlab="Sample size", ylab=TeX("Re($b_1$)"),
         ylim=c(1.5,2.5), type="l", axes=F)
    abline(h=2, col=lineColour, lwd=2)
    axis(side=2)
    box(col="black", lwd=1)
    ...
    plot(Re(data[,3,2]), xlab="Sample size", ylab=TeX("Re($b_1$)"),
         ylim=c(1.5,2.5), type="l", axes=F)
    abline(h=2, col=lineColour, lwd=2)
    axis(side=2)
    axis(side=1)
    ...
    mtext(TeX("Re($b_1$)"), side=1, line=3, at=0.5/2, outer=TRUE);
    
    plot(Im(data[,2,2]), xlab="Sample size", ylab=TeX("Im($b_1$)"),
         ylim=c(-3,0), type="l", axes=F)
    abline(h=-1.5, col=lineColour, lwd=2)
    axis(side=4)
    box(col="black", lwd=1)
    plot(Im(data[,1,2]), xlab="Sample size", ylab=TeX("Im($b_1$)"),
         ylim=c(-3,0), type="l", axes=F)
    abline(h=-1.5, col=lineColour, lwd=2)
    axis(side=4)
    box(col="black", lwd=1)
    plot(Im(data[,3,2]), xlab="Sample size", ylab=TeX("Im($b_1$)"),
         ylim=c(-3,0), type="l", axes=F)
    abline(h=-1.5, col=lineColour, lwd=2)
    axis(side=4)
    axis(side=1)
    box(col="black", lwd=1)
    mtext(TeX("Im($b_1$)"), side=1, line=3, at=1.5/2, outer=TRUE)
}
```
...
```{r parametersUCDV, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot). Scenario 1.", echo=FALSE, fig.height=3, fig.width=8}
...
```{r parametersPC, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot). Scenario 2.", echo=FALSE, fig.height=3, fig.width=8}
...
```{r parametersUCSV, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot). Scenario 3.", echo=FALSE, fig.height=3, fig.width=8}
...
```{r parametersFR, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot). Scenario 4.", echo=FALSE, fig.height=3, fig.width=8}
...
```{r parametersCorError, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot). Scenario 5.", echo=FALSE, fig.height=3, fig.width=8}
...
```{r parametersHVError, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot). Scenario 6.", echo=FALSE, fig.height=3, fig.width=8}
