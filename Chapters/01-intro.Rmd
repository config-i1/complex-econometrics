...
\begin{equation}
    \mu_{1} = \mathrm{E}(x) = \mathrm{E}(x_r + i x_i) = \mathrm{E}(x_r) + i \mathrm{E}(x_i) = \mu_{1,r} + i \mu_{1,i},
    (\#eq:crvMomentFirst)
\end{equation}
due to the law of total expectation and because $i$ is a constant. In \@ref(eq:crvMomentFirst), $\mathrm{E}(.)$ is the expectation of a variable, $\mu_{1}$ is the first moment of a complex variable and $\mu_{1,r}$ and $\mu_{1,i}$ are the respective real and imaginary parts of the first moment. In sample, this moment can be calculated as:
\begin{equation}
...
\begin{equation}
    \begin{aligned}
    \sigma_x^2 = & \mathrm{E}(x \tilde{x}) = \mathrm{E}\left(((x_r-\bar{x}_r) + i (x_i-\bar{x}_i))((x_r-\bar{x}_r) - i (x_i-\bar{x}_i))\right) = \\
                & \mathrm{E}((x_r-\bar{x}_r)^2) +  \mathrm{E}((x_i-\bar{x}_i)^2)
    \end{aligned}
...
\begin{equation}
    \begin{aligned}
    \varsigma_x^2 = & \mathrm{E}(x^2) = \mathrm{E}\left(((x_r-\bar{x}_r) + i (x_i-\bar{x}_i))^2\right) = \\
                    & \mathrm{E}((x_r-\bar{x}_r)^2) - \mathrm{E}((x_i-\bar{x}_i)^2) + i2 \mathrm{E}((x_r-\bar{x}_r)(x_i-\bar{x}_i))
    \end{aligned}
...
Moreover, given that any complex variable can be represented in a vector form, the c.r.v. $x$ can be treated as a bivariate random variable, for which a covariance matrix can be calculated, which would be:
\begin{equation}
