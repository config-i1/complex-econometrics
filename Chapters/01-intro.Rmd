...
\end{equation}
Furthermore, the multiplication of a vector by its transposed value in both of these cases is equivalent to the multiplication by the conjugate value. For the vector:
\begin{equation}
...
Given that any complex number can be represented as a vector, the system of equations \@ref(eq:complexFunctionSystem) can be written as:
\begin{equation}
...
\end{equation}
Furthermore, a complex random variable $y_r + i y_i$ can also be represented in a matrix form:
\begin{equation}
    \begin{pmatrix} y_r & - y_i \\ y_i & y_r \end{pmatrix},
    (\#eq:complexVariableMatrix)
\end{equation}
implying that the same function \@ref(eq:complexFunction) can be written as:
\begin{equation}
...
\begin{equation}
    ...
    (\#eq:crvMomentFirst)
\end{equation}
due to the law of total expectation and because $i$ is a constant. In \@ref(eq:crvMomentFirst), $\mathrm{E}(.)$ is the expectation of a variable. In sample, this moment can be calculated as:
\begin{equation}
