...
\end{equation}
which is the square of the magnitude \@ref(eq:complexNumberMagnitude) of both complex variables $z$ and $\tilde{z}$. Conjugation is distributive over addition, subtraction, multiplication and division, meaning that:
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \mathbf{x} = \begin{pmatrix} x_{r,1} + i x_{i,1} \\ x_{r,2} + i x_{i,2} \\ x_{r,3} + i x_{i,3} \end{pmatrix} ,
    (\#eq:complexVector)
\end{equation}
...
\begin{equation}
    \underline{\mathbf{x}} = \begin{pmatrix} x_{r,1} & x_{i,1} \\ x_{r,2} & x_{i,2} \\ x_{r,3} & x_{i,3} \end{pmatrix} ,
    (\#eq:complexVectorMatrix)
\end{equation}
we use the $\underline{}$ symbol below the character to denote the complex variable represented in the form \@ref(eq:complexVectorMatrix). Finally, given the connection of complex numbers and matrices \@ref(eq:complexNumberMatrix), the same complex vector $x$ can be written as a matrix:
\begin{equation}
...
\begin{equation}
    \mathbf{x}^{\top} = \begin{pmatrix} x_{r,1} + i x_{i,1} & x_{r,2} + i x_{i,2} & x_{r,3} + i x_{i,3}
                        \end{pmatrix} ,
...
\begin{equation}
    \underline{\mathbf{x}}^{\top} = \begin{pmatrix} x_{r,1} & x_{r,2} & x_{r,3} \\
...
\end{equation}
We will denote the operation of transposition above with the symbol $\top$ and will call it just "transposition". The transposition that produces the conjugate complex numbers is called "conjugate transposition" and will be denoted as $\prime$ in this monograph.
...
\begin{equation}
    ...
    (\#eq:crvMomentFirst)
\end{equation}
due to the law of total expectation and because $i$ is a constant. In \@ref(eq:crvMomentFirst), $\mathrm{E}(.)$ is the expectation of a variable, $\mu$ is the first moment of a complex variable and $\mu_{r}$ and $\mu_{i}$ are the respective real and imaginary parts of the first moment. In sample, this moment can be calculated as:
\begin{equation}
...
The elliptic uniform distribution of CRV is based on the equation of ellipse:
\begin{equation}
