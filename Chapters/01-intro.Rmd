...
\begin{equation}
    \sigma_x^2 = \mathrm{E}(x \tilde{x}) = \mathrm{E}\left(((x_r-\bar{x}_r) + i (x_i-\bar{x}_i))((x_r-\bar{x}_r) - i (x_i-\bar{x}_i))\right) = \mathrm{E}((x_r-\bar{x}_r)^2) +  \mathrm{E}((x_i-\bar{x}_i)^2)
...
```{r crvMomentSecondVariance, fig.height=4, fig.width=4, fig.cap="", echo=FALSE}
...
\begin{equation}
    \varsigma_x^2 = \mathrm{E}(x^2) = \mathrm{E}\left(((x_r-\bar{x}_r) + i (x_i-\bar{x}_i))^2\right) = \mathrm{E}((x_r-\bar{x}_r)^2) - \mathrm{E}((x_i-\bar{x}_i)^2) + i2 \mathrm{E}((x_r-\bar{x}_r)(x_i-\bar{x}_i))
