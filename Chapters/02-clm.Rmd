...
nsim <- 10000
clsValues <- matrix(NA, nsim, 4, dimnames=list(NULL, c("b1r","b1i","CLSr","CLSi")))
...
# Loop for values of b1r from 0.52 to 2.5 and for b1i from 0.22 to 2.2
l <- 1
for(i in 1:(nsim/100)){
    for(j in 1:(nsim/100)){
...
```{r}
plot(clsValues[,3:4], type="l",
     xlab="Re(CLS loss)", ylab="Im(CLS loss)")
abline(h=0, col="grey", lty=2, lwd=2)
abline(v=0, col="grey", lty=2, lwd=2)
points(Re(clsResult), Im(clsResult), col="red", pch=16)
```

```{r eval=FALSE}
# Calculate distance matrix for all losses (including the CLS point)
...
To produce the 3d surface, we use `plotly` package in R.
