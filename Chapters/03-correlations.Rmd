...
In order to better understand what the conjugate correlation means, we expand the formula \@ref(eq:correlationConventionalEstimate) by substituting $\underline{b_1} = b_{1,r} + i b_{1,i}$ and $\underline{a_1} = a_{1,r} + i a_{1,i}$:
\begin{equation}
    \begin{aligned}
...
2. When the complex variables $\underline{x}$ and $\underline{y}$ have functional relation between them, so that $\underline{a_1} = \frac{1}{\underline{b_1}}$, the coefficient will be equal to one.
