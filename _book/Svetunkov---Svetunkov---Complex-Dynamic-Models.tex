...
\end{equation}
where \(e\) is the Euler's constant. The formula \eqref{eq:complexNumberExponential} is nowadays also called ``Euler's formula''. In this form, the positive real number has \(\phi=0\), while the negative one has \(\phi=\pi\), and all imaginary numbers have an angle dividable by \(\frac{\pi}{2}\). So, for example:
\begin{equation*}
    \begin{aligned}
    41 = & 41 e^{i 0} \\
