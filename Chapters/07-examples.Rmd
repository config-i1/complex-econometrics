...
In the code above we use the `cscale()` function from the `complex` package to scale both parts of the complex variable to abide with the complex-valued modelling principles discussed in Section \@ref(complexModelling). We can then conduct the ADF test:
...
## Seatbels law in Great Britain {#seatbeltsExample}
...
```{r}
# Form the data frame from the used variables
SeatbeltsData <- data.frame(killed=Seatbelts[,"DriversKilled"],
                            injured=Seatbelts[,"drivers"]-
                                Seatbelts[,"DriversKilled"],
                            kms=Seatbelts[,"kms"],
                            PetrolPrice=Seatbelts[,"PetrolPrice"],
                            law=Seatbelts[,"law"])
# Create a data frame with complex variables
SeatbeltsComplex <-
    data.frame(y=complex(real=SeatbeltsData$killed,
                         imaginary=SeatbeltsData$injured),
               kms=SeatbeltsData$kms,
               PetrolPrice=SeatbeltsData$PetrolPrice,
               law=SeatbeltsData$law)
```
...
```{r}
SeatbeltsComplex$seasonal <-
    temporaldummy(SeatbeltsComplex[,1],
...
```{r}
SeatbeltscLR01 <-
    clm(I(clog(y))~log(kms)+log(PetrolPrice)+law+seasonal,
        SeatbeltsComplex)
summary(SeatbeltscLR01)
```
...
<!-- We can conclude based on the, for example, that the introduction of the setabelts law seemed to have reduced the number of drivers killed and injured respectively by approximately 14.6% and 16.3% (on average, keeping all other variables fixed). -->
...
fitted(SeatbeltscLR01) |> cexp() |>
    ts(start=yStart, frequency=yFreq) -> yFitted
...
names(SeatbeltscLRList) <-
    paste0("ARIMA(",apply(cARIMAOrders, 1,
                          paste0, collapse=","),")")
...
for(i in 1:nrow(cARIMAOrders)){
    SeatbeltscLRList[[i]] <-
        clm(I(clog(y))~log(kms)+log(PetrolPrice)+law+seasonal,
            SeatbeltsComplex, orders=cARIMAOrders[i,])
}
```

```{r echo=FALSE}
load("./data/SeatbeltscLRList.Rdata")
```
...
```{r seatbeltsDataModelFinal, fig.cap="Number of drivers killed and injured and the fitted values from the cARIMAX(1,0,1) model.", echo=FALSE}
fitted(SeatbeltscLRList[[iBest]]) |> cexp() |>
    ts(start=yStart, frequency=yFreq) -> yFitted
