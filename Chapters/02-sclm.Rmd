...
In order to do that, we set up several experiments with the following settings...


```{r parametersCorError, fig.cap="Estimation of parameters using OLS, CLS and likelihood and their convergence to the true value of $b_1=2-1.5i$ (orange horizontal lines on the plot)", echo=FALSE}
methods <- c("CLS","OLS","Likelihood")
par(mfcol=c(3,2), mar=rep(0.1,4), oma=c(5,5,2,2), xaxt="s",yaxt="s",cex.main=1.5)
plot(Re(parametersCorError[,2,2]), xlab="Sample size", ylab=TeX("Re($b_1$)"),
     ylim=range(Re(parametersCorError[,,2])), type="l", axes=F)
abline(h=2, col="orange", lwd=2)
axis(side=2)
box(col="black", lwd=1)
mtext(methods[2], side=2, line=3, at=2.5/3, outer=TRUE)

plot(Re(parametersCorError[,1,2]), xlab="Sample size", ylab=TeX("Re($b_1$)"),
     ylim=range(Re(parametersCorError[,,2])), type="l", axes=F)
abline(h=2, col="orange", lwd=2)
axis(side=2)
box(col="black", lwd=1)
...
plot(Re(parametersCorError[,3,2]), xlab="Sample size", ylab=TeX("Re($b_1$)"),
     ylim=range(Re(parametersCorError[,,2])), type="l", axes=F)
abline(h=2, col="orange", lwd=2)
axis(side=2)
axis(side=1)
...
plot(Im(parametersCorError[,2,2]), xlab="Sample size", ylab=TeX("Im($b_1$)"),
     ylim=range(Im(parametersCorError[,,2])), type="l", axes=F)
abline(h=-1.5, col="orange", lwd=2)
axis(side=4)
box(col="black", lwd=1)
plot(Im(parametersCorError[,1,2]), xlab="Sample size", ylab=TeX("Im($b_1$)"),
     ylim=range(Im(parametersCorError[,,2])), type="l", axes=F)
abline(h=-1.5, col="orange", lwd=2)
axis(side=4)
box(col="black", lwd=1)
plot(Im(parametersCorError[,3,2]), xlab="Sample size", ylab=TeX("Im($b_1$)"),
     ylim=range(Im(parametersCorError[,,2])), type="l", axes=F)
abline(h=-1.5, col="orange", lwd=2)
axis(side=4)
axis(side=1)
...
### Correlation matrix
Finally, as discussed in Subsection \@ref(crvSecondMoment), it is possible to calculate the covariance matrix between two c.r.v. Based on that matrix, we can calculate the correlation matrix, which will summarise all the relations between the real and imaginary parts of $x$ and $y$. This is done by dividing each element of the covariance matrix by geometric means of variances of the variables under consideration. In R, this can be done using `covar()` function from the `complex` package and `cov2cor()` function from the `stats` package:
