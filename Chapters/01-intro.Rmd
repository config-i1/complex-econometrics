...
One of the important definitions in theory of complex numbers is the conjugate complex number. It is the number, for which the imaginary part has the sign opposite to the one of the original variable. For example, the conjugate of the complex variable $\underline{z} = x+ iy$ is $\tilde{\underline{z}} = x- iy$. These numbers are useful because the multiplication of a complex number by its conjugate gives a real number:
\begin{equation}
    \underline{z} \times \tilde{\underline{z}} = (x+ iy) (x- iy) = x^2 + y^2 ,
    (\#eq:complexNumberConjugateMulti)
\end{equation}
which is the square of the magnitude \@ref(eq:complexNumberMagnitude) of both complex variables $\underline{z}$ and $\tilde{\underline{z}}$. Conjugation is distributive over addition, subtraction, multiplication and division, meaning that:
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    \underline{z}{\mathbf{x}}^{\top} = \begin{pmatrix} x_{r,1} + i x_{i,1} & x_{r,2} + i x_{i,2} & x_{r,3} + i x_{i,3}
                        \end{pmatrix} ,
