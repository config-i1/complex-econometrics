...
\begin{equation}
    ...
    (\#eq:complexPower)
\end{equation}
In this function, the complex variable $x_{1,r} + i x_{1,i}$ is transformed non-linearly by taking the complex power of it.
<!-- To better understand what this implies, we can first expand the power into two multiplicants: -->
<!-- \begin{equation} -->
<!--     y_r + i y_i = (x_{r} + i x_{i})^{a_{r}} (x_{r} + i x_{i}) ^{i a_{i}} . -->
<!--     (\#eq:complexPower2) -->
<!-- \end{equation} -->
<!-- Then, consider the exponential form of complex variables in \@ref(eq:complexPower2): -->
<!-- \begin{equation} -->
<!--     \left({y_r^2 + y_i^2}\right)^{\frac{1}{2}} e^{i \mathrm{Arg}(y_r + i y_i)} = \left({x_r^2 + x_i^2}\right)^{\frac{a_{r}}{2}} e^{i \mathrm{Arg}(x_r + i x_i) a_{r}} \left({x_r^2 + x_i^2}\right)^{\frac{i a_{i}}{2}} e^{i \mathrm{Arg}(x_r + i x_i) i a_{i}}  , -->
<!--     (\#eq:complexPowerExp) -->
<!-- \end{equation} -->
<!-- which can be further simplified to: -->
<!-- $\left({y_r^2 + y_i^2}\right)^{\frac{1}{2}} e^{i \mathrm{Arg}(y_r + i y_i)} = \left({x_r^2 + x_i^2}\right)^{\frac{a_{r}}{2}} e^{- a_{i} \mathrm{Arg}(x_r + i x_i) } e^{\frac{i a_{i}}{2} \ln \left({x_r^2 + x_i^2}\right)} e^{i \mathrm{Arg}(x_r + i x_i) a_{r}} $ -->

$\ln (y_r + i y_i) = (a_{r} + i a_{i}) \ln (x_{r} + i x_{i})$
$\left(\frac{1}{2} \ln \left({y_r^2 + y_i^2}\right) + {i \mathrm{Arg}(y_r + i y_i)})\right) = (a_{r} + i a_{i}) \left(\frac{1}{2} \ln \left({x_r^2 + x_i^2}\right) + {i \mathrm{Arg}(x_r + i x_i)})\right)$


The function \@ref(eq:complexPower) can also be linearised to get:
\begin{equation*}
    \ln (y_r + i y_i) = (a_{r} + i a_{i}) \ln (x_{r} + i x_{i}) .
\end{equation*}
this linearisation will be useful when we will need to estimate models like this on real data.
