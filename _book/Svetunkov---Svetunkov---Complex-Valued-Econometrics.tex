...
\begin{verbatim}
##           x_r       x_i
## x_r 23.029460  5.177241
## x_i  5.177241 91.454629
\end{verbatim}
...
\begin{verbatim}
##                     x                   y
## x -55.04338- 4.60502i -51.54681+40.05824i
## y -51.54681+40.05824i   3.62932+72.15798i
\end{verbatim}
...
\begin{verbatim}
##        x_r    x_i    y_r   y_i
## x_r  1.000 -0.060  0.735 0.699
## x_i -0.060  1.000 -0.718 0.670
## y_r  0.735 -0.718  1.000 0.032
## y_i  0.699  0.670  0.032 1.000
\end{verbatim}
...
In principles, it can be shown that the main properties of real-valued AR discussed by \citet{Box1976} hold widely for the cAR processes: cACF will decline exponentially starting from the lag p.
...
\NormalTok{y }\OtherTok{\textless{}{-}} \FunctionTok{rcnorm}\NormalTok{(obs, }\DecValTok{0}\NormalTok{, }\AttributeTok{sigma2=}\DecValTok{25}\NormalTok{, }\AttributeTok{varsigma2=}\DecValTok{16}\SpecialCharTok{+}\NormalTok{9i)}
\CommentTok{\# Initial value}
\NormalTok{y[}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ b0 }\SpecialCharTok{+}\NormalTok{ y[}\DecValTok{1}\NormalTok{]}
\CommentTok{\# The cAR(1)}
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{2}\SpecialCharTok{:}\NormalTok{obs)\{}
\NormalTok{    y[i] }\OtherTok{\textless{}{-}}\NormalTok{ phi1 }\SpecialCharTok{*}\NormalTok{ y[i}\DecValTok{{-}1}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ y[i] }\SpecialCharTok{+}\NormalTok{ b0}
\NormalTok{\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cacf}\NormalTok{(y, }\AttributeTok{method=}\StringTok{"conjugate"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cpacf}\NormalTok{(y, }\AttributeTok{method=}\StringTok{"conjugate"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Shift the variable to create the lagged explanatory one}
\NormalTok{complexData }\OtherTok{\textless{}{-}} \FunctionTok{xregExpander}\NormalTok{(}\FunctionTok{complex2vec}\NormalTok{(y), }\AttributeTok{lags=}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{)}
\CommentTok{\# Form the complex data frame}
\NormalTok{complexData }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{y=}\FunctionTok{vec2complex}\NormalTok{(complexData[,}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{)]),}
                          \AttributeTok{yLag1=}\FunctionTok{vec2complex}\NormalTok{(complexData[,}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{)}\SpecialCharTok{+}\DecValTok{1}\NormalTok{]))}
\FunctionTok{colnames}\NormalTok{(complexData) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"y"}\NormalTok{,}\StringTok{"yLag1"}\NormalTok{)}
\CommentTok{\# Fit the model}
\NormalTok{cAR1Model }\OtherTok{\textless{}{-}} \FunctionTok{clm}\NormalTok{(y}\SpecialCharTok{\textasciitilde{}}\NormalTok{., complexData, }\AttributeTok{subset=}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{80}\NormalTok{))}
...
## Coefficients:
##               Estimate Std. Error Lower 2.5% Upper 97.5%  
## (Intercept)_r 103.4139    17.6311    68.3131    138.5147 *
## (Intercept)_i  52.3245     9.0897    34.2284     70.4206 *
## yLag1_r         0.4822     0.0846     0.3138      0.6505 *
## yLag1_i         0.2033     0.0436     0.1165      0.2901 *
## 
## Error covariance matrix:
##         e_r    e_i
## e_r 17.1478 4.1270
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Vector of forecasts}
\NormalTok{yForecast }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"complex"}\NormalTok{, }\DecValTok{20}\NormalTok{)}
\CommentTok{\# The data frame for the future values}
\NormalTok{complexData20 }\OtherTok{\textless{}{-}} \FunctionTok{tail}\NormalTok{(complexData,}\DecValTok{20}\NormalTok{)}
\NormalTok{complexData20}\SpecialCharTok{$}\NormalTok{y[] }\OtherTok{\textless{}{-}} \ConstantTok{NA}
\NormalTok{complexData20}\SpecialCharTok{$}\NormalTok{yLag1[}\DecValTok{2}\SpecialCharTok{:}\DecValTok{20}\NormalTok{] }\OtherTok{\textless{}{-}} \ConstantTok{NA}

\CommentTok{\# Iteative loop to produce one{-}step{-}ahead forecasts}
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{)\{}
\NormalTok{    cAR1ModelForecast }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(cAR1Model,}
                                 \AttributeTok{newdata=}\NormalTok{complexData20[i,])}
\NormalTok{    yForecast[i] }\OtherTok{\textless{}{-}}\NormalTok{ cAR1ModelForecast}\SpecialCharTok{$}\NormalTok{mean}
    \ControlFlowTok{if}\NormalTok{(i}\SpecialCharTok{\textless{}}\DecValTok{20}\NormalTok{)\{}
        \CommentTok{\# Write down the forecast as the new lagged value}
\NormalTok{        complexData20}\SpecialCharTok{$}\NormalTok{yLag1[i}\SpecialCharTok{+}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ cAR1ModelForecast}\SpecialCharTok{$}\NormalTok{mean}
\NormalTok{    \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

Unfortunately, the process of forecast generation is not yet automated in the functions of the \texttt{complex} package, so the code above demonstrates how this can be done manually in an iterative fashion, making sure that there is no holdout values leackage in the process. The forecasts are shown in Figure \ref{fig:complexAR1Forecast}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Produce the plot of the data and the forecasts}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mfcol=}\FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{),}\AttributeTok{mar=}\FunctionTok{c}\NormalTok{(}\DecValTok{4}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{))}
\FunctionTok{plot}\NormalTok{(}\FunctionTok{Re}\NormalTok{(y), }\AttributeTok{type=}\StringTok{"l"}\NormalTok{, }\AttributeTok{xlab=}\StringTok{"Time"}\NormalTok{, }\AttributeTok{main=}\StringTok{""}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(}\DecValTok{81}\SpecialCharTok{:}\DecValTok{100}\NormalTok{, }\FunctionTok{Re}\NormalTok{(yForecast), }\AttributeTok{col=}\StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd=}\DecValTok{2}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(}\FunctionTok{Im}\NormalTok{(y), }\AttributeTok{type=}\StringTok{"l"}\NormalTok{, }\AttributeTok{xlab=}\StringTok{"Time"}\NormalTok{, }\AttributeTok{main=}\StringTok{""}\NormalTok{)}
\FunctionTok{lines}\NormalTok{(}\DecValTok{81}\SpecialCharTok{:}\DecValTok{100}\NormalTok{, }\FunctionTok{Im}\NormalTok{(yForecast), }\AttributeTok{col=}\StringTok{"blue"}\NormalTok{, }\AttributeTok{lwd=}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\end{equation}
Given that the basic assumptions discussed in Section @ref() hold for the ``true model'', the covariance is simplified to:
\begin{equation}
