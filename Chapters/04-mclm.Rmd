...
In case of complex linear regression, it is possible to introduce the dummy variables in the same way as in the conventional model, by adding real-valued variables. However, in addition to that, it is also possible to encode a complex dummy variable, which would have two features in it at the same time, e.g. $colourRed +i \times sizeSmall$, where both `colourRed` and `sizeSmall` would be dummy variables.
