...
\begin{equation}
    \begin{aligned}
    \sigma_x^2 = & \mathrm{E}(x \tilde{x}) = \mathrm{E}\left(((x_r-\mu_{r}) + i (x_i-\mu_{i}))((x_r-\mu_{r}) - i (x_i-\mu_{i}))\right) = \\
...
An alternative way to measure the variability of a complex random variable is by calculating the so called "pseudo-variance" [@reference], which can be obtained by applying the conventional formula of variance directly to the c.r.v.:
\begin{equation}
    \begin{aligned}
    \varsigma_x^2 = & \mathrm{E}(x^2) = \mathrm{E}\left(((x_r-\mu_{r}) + i (x_i-\mu_{i}))^2\right) = \\
...
\begin{equation}
    \Sigma_x = \begin{pmatrix} \sigma_{x_r}^2 & \sigma_{x_r, x_i} \\ \sigma_{x_r, x_i} & \sigma_{x_i}^2 \end{pmatrix} .
...
\begin{equation}
    \mathrm{GV} = |\Sigma_x| = \sigma_{x_r}^2 \sigma_{x_i}^2 - \sigma_{x_r, x_i}^2 .
...
\begin{equation}
    \begin{aligned}
        \hat{\sigma}_{x_r}^2 = & \frac{1}{n} \sum_{j=1}^n (x_{r,j}-\bar{x}_{r,j})^2 \\
        \hat{\sigma}_{x_i}^2 = & \frac{1}{n} \sum_{j=1}^n (x_{i,j}-\bar{x}_{i,j})^2 \\
        \hat{\sigma}_{x_r, x_i} = & \frac{1}{n} \sum_{j=1}^n (x_{r,j}-\bar{x}_{r,j})(x_{i,j}-\bar{x}_{i,j})^2
    \end{aligned}
