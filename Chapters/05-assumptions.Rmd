...
x2 <- complex(real=Im(x1)*1.5 + rnorm(obs,0,1), imaginary=Re(x1)*2.5 + rnorm(obs,0,1))
# Generate parameters
b0 <- 100 - 150i
b1 <- 2.5 + 1.5i
b2 <- 1.5 - 0.75i
...
y <- b0 + b1 * x1 + b2 * x2 + e
# Form data frame with the variables
xreg <- data.frame(y=y,x1=x1,x2=x2)
```
...
```{r}
ccor(xreg,method="conjugate")
ccor(xreg,method="direct")
ccor(xreg,method="pearson")
covar(xreg) |> cov2cor()
```
